...
class NaiveBayesClassifier(object):
    ...
    def fit(self, feature, label):
        ...
        for Label in label_set:
            self.label_prob[Label] = len(label[label == Label]) / len(label)
        ...
        for Label in label_set:
            ...
            for i in range(feature_num):
                ...
                # 计算当前标签对应的各个特征的概率
                for Feature in feature_set:
                    label_count[Feature] = label_count[Feature] / total_feature_lens
        ...
        for i in range(test_num):
            ...
            for Label in self.label_prob.keys():
                ...
                for j in range(feature_num):
                    p *= self.condition_prob[Label][j][feat[j]]
                prob[Label] = p
            # 选择概率最大的标签
            index = max(prob, key=prob.get)
            result.append(index)
