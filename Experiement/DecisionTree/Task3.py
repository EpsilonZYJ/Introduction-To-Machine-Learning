...
class DecisionTree(object):
    ...
    def predict(self, feature):
        ...
        '''
        #************* Begin ************#
        def predictSingle(tree, feature):
            key, value = list(tree.items())[0]
            if isinstance(value[feature[key]], dict):
                return predictSingle(value[feature[key]], feature)
            else:
                return value[feature[key]]

        prediction = np.zeros(len(feature))
        for i in range(len(feature)):
            prediction[i] = predictSingle(self.tree, feature[i])
        return prediction
        #************* End **************#